...
def generate_circuit(fuzzy_partitions):
    ...
    for partition in fuzzy_partitions:
        ...
        qc.add_register(QuantumRegister(math.ceil(math.log(partition.len_partition(), 2))+1, name=partition.name))
        Qregisters.append(QuantumRegister(math.ceil(math.log(partition.len_partition(), 2))+1, name=partition.name))
